...
def init_preprocess_func(model_name):
    """
    This block of code is used to initialize the input sizes
    of the images for specific models. Because specific models
    are trained using images of specific sizes. If any new 
    ...
    return pre_func[model_name]

def get_models(model_name, stage_no, path_dict):
    """
    ...
    """
    model = load_model(path_dict["model_path"]+"stage{}/".format(stage_no)+"{}_model_stage_{}.h5".format(model_name, stage_no))
    return model

#Plot loss vs epochs
def plt_epoch_error(history_df, model_name, stage_no, path_dict):
    """
    ...
    plt.savefig(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_history_stage_{}.png'.format(model_name,stage_no))
    print("\nFile saved at this location: "+path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_history_stage_{}.png'.format(model_name,stage_no))
...
def get_complete_report(y_true, y_pred, model_name, stage_no, class_indices, path_dict):
    """
    ...
    cm.save_html(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_detailed_metrics_analysis_stage_{}'.format(model_name,stage_no))

def plot_confusion_matrix(y_true, y_pred, labels, model_name, stage_no, path_dict):
    """
    ...
    plt.savefig(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_cm_matrix_stage_{}.png'.format(model_name,stage_no))
    ...
    plt.savefig(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_precision_matrix_stage_{}.png'.format(model_name,stage_no))
    ...
    plt.savefig(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_recall_matrix_stage_{}.png'.format(model_name,stage_no))


def predict_on_test(input_params, stage_no, path_dict):
    """
    ...
    df_test=load_data("test", path_dict)
    
    test_datagen = ImageDataGenerator(preprocessing_function=init_preprocess_func(input_params['model_name']))

    test_generator = test_datagen.flow_from_dataframe(dataframe=df_test,
                                                      directory=path_dict["source"],
                                                      target_size=init_sizes(input_params['model_name']),
                                                      x_col="filenames",
                                                      y_col="class_label",
                                                      batch_size=1,
    ...
    model = get_models(input_params['model_name'], stage_no, path_dict)
    ...
    df_test.to_csv(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_predictions_stage_{}.csv'.format(input_params['model_name'],stage_no))
    ...
    df_cm.to_csv(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_cm_stage_{}.csv'.format(input_params['model_name'],stage_no))
    ...
    df_rep.to_csv(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_class_report_stage_{}.csv'.format(input_params['model_name'],stage_no))
    print('Classification report prepared and saved..')

    plot_confusion_matrix(y_true, y_pred, list(test_generator.class_indices.keys()), input_params['model_name'], stage_no, path_dict)

    #General Metrics
    df_metrics = get_metrics(y_true, y_pred)
    df_metrics.to_csv(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_metrics_stage_{}.csv'.format(input_params['model_name'],stage_no))

    history_df=pd.read_csv(path_dict["model_path"]+"stage{}/".format(stage_no)+"{}_history_stage_{}.csv".format(input_params['model_name'], stage_no))

    #Get the train vs validation loss for all epochs
    plt_epoch_error(history_df, input_params['model_name'], stage_no, path_dict)

    #Generate a complete report and save it as an HTML file in the evaluation folder location
    get_complete_report(y_true, y_pred, input_params['model_name'], stage_no, class_indices, path_dict)
