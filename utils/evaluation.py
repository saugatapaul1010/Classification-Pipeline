...
def plot_confusion_matrix(test_y, predict_y, test_generator, model_name, stage_no, path_dict):
    """
...
def predict_on_test(model_name, size_dict, stage_no, path_dict):
    """
    ...
    model = get_models(model_name, stage_no, path_dict)
    ...
    df_test.to_csv(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_predictions_stage_{}.csv'.format(model_name,stage_no))
    ...
    df_cm.to_csv(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_cm_stage_{}.csv'.format(model_name,stage_no))
    ...
    df_rep.to_csv(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_class_report_stage_{}.csv'.format(model_name,stage_no))
    print('Classification report prepared and saved..')

    plot_confusion_matrix(y_true, y_pred, test_generator, model_name, stage_no, path_dict)

    #General Metrics
    df_metrics = get_metrics(y_true, y_pred)
    df_metrics.to_csv(path_dict["eval_path"]+"stage{}/".format(stage_no)+'{}_metrics_stage_{}.csv'.format(model_name,stage_no))

    history_df=pd.read_csv(path_dict["model_path"]+"stage{}/".format(stage_no)+"{}_history_stage_{}.csv".format(model_name, stage_no))

    #Get the train vs validation loss for all epochs
    plt_epoch_error(history_df,model_name,stage_no, path_dict)

    #Generate a complete report and save it as an HTML file in the evaluation folder location
    get_complete_report(y_true, y_pred, model_name, stage_no, class_indices, path_dict)
