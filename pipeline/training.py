...
def save_summary(model, model_name, stage_no):
    ...
    with open(model_path+"{}_model_summary_stage_{}.txt".format(model_name, stage_no), "w") as f:
        with redirect_stdout(f):
            model.summary()
        
def save_params(input_params):
    """
    ...
    with open(eval_path + '/hyperparameters.csv', 'w') as f:
...
def callbacks_list(input_params, stage_n):
    ...
    filepath = model_path+"{}_weights_stage_{}.hdf5".format(input_params['model_name'], stage_n)
...
def no_of_classes():
    ...
    df = pd.read_msgpack(df_path+"train.msgpack")
...
def train_stage1(input_params):
    ...
    evaluation.predict_on_test(input_params['model_name'],init_sizes(),1)
...
def train_stage2(input_params, stage1_params, model_stg2):
    ...
    evaluation.predict_on_test(input_params['model_name'],init_sizes(),2)
